// Интерфейс: одно меню модификации, характеристики, панель действий, HUD.
import { reticleSVG } from './reticle.js';

const STATS = [
  { k: 'weight', l: 'Масса', u: 'кг', f: (v) => (v / 1000).toFixed(2), better: -1 },
...
const CHIP = { 'recoilV%': 'отдача', 'recoilH%': 'увод', ergo: 'эрг.', loud: 'дБ', weight: 'г', adsTime: 'мс', flash: 'вспышка', moa: 'MOA', velocity: 'м/с', mag: 'патр.', length: 'мм' };
const CHIP_BETTER = { 'recoilV%': -1, 'recoilH%': -1, ergo: 1, loud: -1, weight: -1, adsTime: -1, flash: -1, moa: -1, velocity: 1, mag: 1, length: -1 };

const MODE = { safe: 'ПРЕДОХР.', semi: 'ОДИНОЧНЫЙ', auto: 'АВТО' };
...
export class UI {
  constructor(app) {
    ...
    // карточка оружия + характеристики
    ...
    reset.onclick = () => { this.open = null; app.resetCfg(); };
  ...
  buildBar() {
    ...
    const b = (id, label, fn, cls = '') => {
      ...
      e.dataset.id = id;
      e.addEventListener('click', (ev) => { ev.preventDefault(); a.audio.init(); fn(); });
      this.bar.appendChild(e);
      return e;
    };
    this.fireBtn = b('fire', 'Огонь', () => {}, 'fire');
    this.fireBtn.addEventListener('pointerdown', (e) => { e.preventDefault(); a.triggerDown(); });
  ...
  refresh() { this.renderMods(); this.renderStats(); this.hud(); }

  slotAvailable(slot) {
    const asm = this.app.asm;
    if (slot.rails) return slot.rails.some((r) => asm.mounts.has(r));
    return asm.mounts.has(slot.mount);
  }
  ...
  renderMods() {
    ...
    for (const slot of def.slots) {
      const opts = asm.slotOptions(slot);
      if (!opts.length) continue;
      if (!this.slotAvailable(slot)) { hiddenSlots.push(slot.label); continue; }
      if (!groups.has(slot.group)) groups.set(slot.group, []);
      groups.get(slot.group).push({ slot, opts });
    }
    for (const [g, list] of groups) {
      const ge = el('div', 'm-group');
      ge.appendChild(el('div', 'm-gt', esc(g)));
      for (const { slot, opts } of list) {
        const cur = cfg[slot.id]?.id;
        const part = cur && asm.part(cur);
        const row = el('button', 'm-row' + (this.open === slot.id ? ' open' : '') + (part ? '' : ' empty'));
        row.innerHTML = `<span class="m-l">${esc(slot.label)}</span><span class="m-v">${esc(part ? part.name : 'нет')}</span><span class="m-c">${this.open === slot.id ? '–' : '+'}</span>`;
        row.onclick = () => {
          this.open = this.open === slot.id ? null : slot.id;
          if (this.open) this.app.focusSlot(slot.id); else this.app.unfocus();
          this.renderMods();
        };
        ge.appendChild(row);
        if (this.open === slot.id) ge.appendChild(this.renderOptions(slot, opts, cur));
      }
      body.appendChild(ge);
    }
  ...
  renderOptions(slot, opts, cur) {
    const a = this.app;
    const box = el('div', 'm-opts');
    // положение на планке
    const ri = cur && a.railInfo(slot.id);
    if (ri && ri.n > 1) {
      const pos = el('div', 'm-pos');
      const l = el('button', 'btn sm', '◀'), r = el('button', 'btn sm', '▶');
      const isStock = slot.id === 'stock';
      l.onclick = () => { a.railMove(slot.id, isStock ? 1 : -1); };
      r.onclick = () => { a.railMove(slot.id, isStock ? -1 : 1); };
    ...
    const item = (part) => {
      ...
      const why = part && !on ? this.canPut(slot, part) : null;
      const e = el('button', 'm-opt' + (on ? ' on' : '') + (why ? ' off' : ''));
      if (!part) { e.innerHTML = `<div class="o-n">Не устанавливать</div>`; }
      else {
      ...
      e.disabled = !!why;
      e.onclick = () => { if (!on) a.setPart(slot.id, part ? part.id : null); };
      box.appendChild(e);
    };
    if (!slot.required) item(null);
  ...
  tip(text, x, y) {
    if (!text) { this.tipEl.classList.remove('on'); return; }
