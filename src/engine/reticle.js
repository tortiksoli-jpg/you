// Сетки прицелов: текстура для «проекции в бесконечность» через стенсил линзы
// и SVG для увеличенного вида.
import * as THREE from 'three';

// field — сколько угловых минут охватывает текстура
const R = {
...
function draw(kind, g, s, col) {
  const c = s / 2;
  const u = s / (R[kind]?.field || 16); // пикселей на MOA
  g.strokeStyle = col; g.fillStyle = col; g.lineCap = 'round';
  const glow = (fn, blur) => { g.save(); g.shadowColor = col; g.shadowBlur = blur; fn(); g.restore(); };
  if (kind === 'dot') {
    glow(() => { g.beginPath(); g.arc(c, c, 2.2 * u, 0, 7); g.fill(); }, 3 * u);
  } else if (kind === 'holo') {
    glow(() => {
      g.lineWidth = 1.4 * u;
    ...
    glow(() => {
      g.lineWidth = 1.2 * u;
      g.beginPath(); g.moveTo(c - 12 * u, c + 8 * u); g.lineTo(c, c + 1 * u); g.lineTo(c + 12 * u, c + 8 * u); g.stroke();
      g.beginPath(); g.arc(c, c - 3 * u, 1.3 * u, 0, 7); g.fill();
      g.beginPath(); g.moveTo(c, c - 22 * u); g.lineTo(c, c - 12 * u); g.stroke();
    }, 1.2 * u);
  } else if (kind === 'chevron') {
    glow(() => {
      g.lineWidth = 1.2 * u;
      g.beginPath(); g.moveTo(c - 3.5 * u, c + 5 * u); g.lineTo(c, c); g.lineTo(c + 3.5 * u, c + 5 * u); g.stroke();
    }, 1.2 * u);
    g.strokeStyle = 'rgba(10,10,10,.9)'; g.lineWidth = 0.6 * u;
    g.beginPath(); g.moveTo(c, c + 6 * u); g.lineTo(c, c + 26 * u); g.stroke();
    for (let i = 1; i <= 4; i++) { const w = (6 - i) * 1.6 * u; g.beginPath(); g.moveTo(c - w, c + (6 + i * 4.5) * u); g.lineTo(c + w, c + (6 + i * 4.5) * u); g.stroke(); }
  } else if (kind === 'lpvo') {
    g.strokeStyle = 'rgba(10,10,10,.9)'; g.lineWidth = 0.9 * u;
    for (const [dx, dy] of [[-1, 0], [1, 0], [0, 1], [0, -1]]) { g.beginPath(); g.moveTo(c + dx * 34 * u, c + dy * 34 * u); g.lineTo(c + dx * 70 * u, c + dy * 70 * u); g.stroke(); }
    g.lineWidth = 0.4 * u;
    for (const [dx, dy] of [[-1, 0], [1, 0], [0, 1]]) { g.beginPath(); g.moveTo(c + dx * 10 * u, c + dy * 10 * u); g.lineTo(c + dx * 34 * u, c + dy * 34 * u); g.stroke(); }
    glow(() => { g.lineWidth = 1.2 * u; g.beginPath(); g.arc(c, c, 18 * u, 0, 7); g.stroke(); g.beginPath(); g.arc(c, c, 1.2 * u, 0, 7); g.fill(); }, 1.5 * u);
  } else if (kind === 'pso1') {
    g.strokeStyle = col; g.lineWidth = 3 * u * 0.6;
    const ch = (x, y, k = 1) => { g.beginPath(); g.moveTo(c + x - 6 * u * k, c + y + 8 * u * k); g.lineTo(c + x, c + y); g.lineTo(c + x + 6 * u * k, c + y + 8 * u * k); g.stroke(); };
    glow(() => { ch(0, 0, 1.4); ch(0, 30 * u, 1); ch(0, 50 * u, 1); ch(0, 70 * u, 1); g.beginPath(); g.moveTo(c - 110 * u, c); g.lineTo(c - 20 * u, c); g.moveTo(c + 20 * u, c); g.lineTo(c + 110 * u, c); g.stroke(); }, 1.2 * u);
    g.lineWidth = 1.5 * u;
    for (let i = -10; i <= 10; i++) if (i) { g.beginPath(); g.moveTo(c + i * 10 * u, c); g.lineTo(c + i * 10 * u, c - (i % 5 ? 4 : 8) * u); g.stroke(); }
    // дальномерная шкала
    g.beginPath(); g.moveTo(c - 140 * u, c + 100 * u); g.lineTo(c - 60 * u, c + 100 * u); g.stroke();
    g.beginPath(); for (let i = 0; i <= 20; i++) { const x = c - 140 * u + i * 4 * u, y = c + 100 * u - 60 * u / (1 + i * 0.35); i ? g.lineTo(x, y) : g.moveTo(x, y); } g.stroke();
  }
}
...
export function reticleTexture(kind) {
  ...
  draw(kind, cv.getContext('2d'), s, R[kind]?.color || '#ff2a1a');
...
// Меш сетки: плоскость на дистанции dist (мм, в координатах модуля) по оси прицела.
// Виден только сквозь линзу этого прицела (стенсил-тест).
export function reticleMesh(kind, sight, ref, dist = 60000) {
  ...
  const m = new THREE.Mesh(new THREE.PlaneGeometry(size, size), new THREE.MeshBasicMaterial({
    ...
    blending: kind === 'chevron' || kind === 'lpvo' || kind === 'pso1' ? THREE.NormalBlending : THREE.AdditiveBlending,
...
// SVG сетки для увеличенного вида (оверлей поверх экрана).
export function reticleSVG(kind, zoom = 1) {
  const red = '#ff2d1c';
  const blk = '#0b0b0b';
  const glow = `<filter id="gl"><feGaussianBlur stdDeviation="1.2"/><feMerge><feMergeNode/><feMergeNode in="SourceGraphic"/></feMerge></filter>`;
  if (kind === 'chevron') {
    let bdc = '';
    for (let i = 1; i <= 5; i++) { const w = (6 - i) * 7; bdc += `<line x1="${-w}" y1="${22 + i * 20}" x2="${w}" y2="${22 + i * 20}" stroke="${blk}" stroke-width="1.6"/><text x="${w + 6}" y="${26 + i * 20}" font-size="9" fill="${blk}">${i + 3}</text>`; }
